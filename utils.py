    ...
    if os.path.exists(templates_dir):
        for d in os.listdir(templates_dir):
            if os.path.isdir(os.path.join(templates_dir, d)):
                items.append((d, d, f"Select {d} Race"))
    if not items:
        ...
        if os.path.exists(race_dir):
            for d in os.listdir(race_dir):
                if os.path.isdir(os.path.join(race_dir, d)):
                    items.append((d, d, f"Select {d} variant"))
    if not items:
        items.append(("NONE", "None", "No subfolders found"))
    return items
...
def get_model_items(self, context):
    ...
    if race and race != "NONE":
        target_dir = os.path.join(get_templates_dir(), race)
